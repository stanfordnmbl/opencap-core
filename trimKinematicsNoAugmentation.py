...
# sessionName = 'OpenCapData_3f3e9f24-aa8e-40c7-97dd-67325e70c2cd'
sessionName = '3f3e9f24-aa8e-40c7-97dd-67325e70c2cd'
initialTime = 0.0
finalTime = 5.42
trialName = 'jump'
...
postAugmentationDir = os.path.join(sessionDir, markerDataFolderName, 
                                   'PostAugmentation')
# TODO
parkerTempPath = os.path.join(postAugmentationDir,trialName,trialName + ".trc")
...
dataToTrim = osim.TimeSeriesTableVec3(parkerTempPath)
#Delete any columns that don't end in "_study"
col_labels = dataToTrim.getColumnLabels()  

#remove columns after the target index
for label in col_labels:
    if not "_study" in label: 
...
if rotateData:
    # Read in the data. Decided to use pd, but could use OpenSim API.
...
# Import TRC file
trc_file = utilsDataman.TRCFile(pathOutputFiles[trialName])
...
for marker in markers:
    for time in times:
        vec3 = trc_file.marker_at(marker,time)
        vertical_offset = min(vec3[1],vertical_offset)

trc_file.offset('y', -(vertical_offset-0.01))
# %% Return trimmed .trc file with offset
trc_file.write(pathAugmentedOutputFiles[trialName])
